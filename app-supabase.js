// ── SUPABASE: Auditoria, login/logout, avulso, pag.rápido ──
// NOTA: saveEntidade, loadAppState, saveCounters, deleteEntidade,
// iniciarBackupAutomatico e doLogin estão definidos em app-extras.js
...
function salvarPagRapido(){
  ...
  const titulo = {
    id: newId('tit'), forn, tipo: 'Avulso', ref: desc, valor,
    emissao: data, venc: data, status: 'Pago', placa: '-',
    obs: `Pagamento Rápido — ${forma}`, categoria: cat, competencia: data.slice(0,7),
  };
...
function salvarAvulso(){
  ...
  vencs.forEach((el,i) => {
    ...
    Titulos.unshift({
      id: newId('tit'), forn, tipo: 'Avulso',
      ref: (ref||desc) + label,
      valor: Math.round(parseFloat(vals[i].value)*100)/100,
      emissao: compData, venc: el.value, status: 'Pendente', placa: '-',
      obs: desc + (ref ? ` — Ref: ${ref}` : '') + ` — ${forma}`,
      categoria: cat, competencia: compYM,
    });
  });
